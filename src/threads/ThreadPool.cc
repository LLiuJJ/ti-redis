    ...
    while (working_)
    {
...

        {
            ...
            { 
                ...
                return this->shutdown_ || !tasks_.empty(); // which returns ​false if the waiting should be continued 
            });
            -- waiters_;
...
void ThreadPool::_MonitorRoutine()
{
    while (!shutdown_)
    {
        ...
        while (nw -- > maxIdleThread_)
        {
            ...
            cond_.notify_one(); // notify_one unblocks one of the waiting threads
            ++ pendingStopSignal_;
        }
    }
}
